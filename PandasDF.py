...
snow_df_bestpath = session.table('CUSTOMERAI.MAIN.VW_CHANNEL_DISPLAY_BESTPATH')
snow_df_bestpath1 = session.table('CUSTOMERAI.MAIN.VW_CHANNEL_DISPLAY_BESTPATH1')
snow_df_importance = session.table('CUSTOMERAI.MAIN.VW_CHANNEL_IMPORTANCE_RESULTS')
snow_df_leastpath = session.table('CUSTOMERAI.MAIN.VW_CHANNEL_DISPLAY_LEAST_PATH')
snow_df_q_matrix_csv = session.table('CUSTOMERAI.MAIN.VW_CHANNEL_DISPLAY_MATRIX')
# snow_df_salesai_bestpath1 = session.table('CUSTOMERAI.MAIN.vw_salesai_bestpath1')
snow_df_salesai_incompletepath = session.table('CUSTOMERAI.MAIN.VW_CHANNEL_DISPLAY_INCOMPLETE_PATH')


npc = session.sql('SELECT _C0,MEMBERSHIP, EMAIL, CUSTOMER_SERVICE, WEBSITE, SOCIAL_MEDIA, STORE, EVENT_FORM FROM CUSTOMERAI.MAIN."CHANNEL_DISPLAY_MATRIX"')
npc_df = npc.toPandas()
